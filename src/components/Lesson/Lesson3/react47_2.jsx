...
import term from '../../../asset3/Misc/เสี่ยง2.png';
...
const App = () => {
  const [showNextButton, setShowNextButton] = useState(false);
      useEffect(() => {
      document.body.className = 'page-bg472';
    ...
    useEffect(() => {
      ...
      const updated = Array.from(new Set([...visited, "p47_2"]));
      localStorage.setItem("visitedPages", JSON.stringify(updated));
    }, []);

    useEffect(() => {
  localStorage.setItem("visited_p47_2", "true");
      const visitedAll = 
  ...
  return (
    <div>
      ...
      <Link className="btnback472" to="/p47_1" data-discover="true">ย้อนกลับ</Link>
      <div className="center472">
        <h2>
          ความสูงของตัวรถ ทำให้บดบัง
          <br />ทัศนวิสัยในการมองเห็น โดย
          <br />เฉพาะในระยะประชิด
        </h2>
        <img src={character} />
      </div>
      <div className="carder472">
        <img className="buntookbon472" src={buntookbon} />
      </div>
      <div className="left-triangle472"></div>
      <div className="right-triangle472"></div>
      <div className="bottom-triangle472"></div>
      <div className="top-square472"></div>
